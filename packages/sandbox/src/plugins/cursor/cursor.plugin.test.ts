...
describe('expandPlugins — cursor', () => {
  ...
  it('emits no commands and no bootstrap when host Cursor is undetectable', async () => {
    vi.spyOn(hostCursor, 'readHostCursor').mockReturnValue(null);
    const expanded = await expandPlugins(project, emptyUser, ctx);
  ...
  it('emits a post-lockdown pre-warm command (default user, no bootstrap) when host Cursor is detected', async () => {
  ...
  it('uses the x64 arch positional when host is x64', async () => {
    vi.spyOn(hostCursor, 'readHostCursor').mockReturnValue({
      commit: '3e548838cf824b70851dd3ef27d0c6aae371b3f6',
      arch: 'x64',
    });
